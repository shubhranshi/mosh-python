# Python Closures
# Before seeing what a closure is, we have to first understand what
# nested functions and non-local variables are.
...
# outside their scope. This can be illustrated by the following example:
...
# but not outside of its body. Hence, here, innerFunction() is treated as nested Function
# which uses text as non-local variable.

# Python Closures
# A Closure is a function object that remembers values in enclosing scopes even if they
# are not present in memory.
#  * It is a record that stores a function together with an environment: a mapping associating
# each free variable of the function (variables that are used locally but defined in an enclosing scope)
...
#  through the closure’s copies of their values or references, even when the function is invoked
#  outside their scope.
...
# 2) The function innerFunction has its scope only inside the outerFunction.
